...
let new_var =
  let r = ref 0 in fun () -> let i = string_of_int (!r) in incr r; "_x"^i

module Raw = struct

  type 'a kind =
    | RBase
    | RArrow of 'a kind * 'a kind
    | RPi of string * 'a kind * 'a kind
    | RProd of 'a kind * 'a kind
    | RSigma of string * 'a kind * 'a kind
    | RSingle of 'a
...
let rec pre_h_typ_rec h_kind (x : fvar) = function
  ...
  | BVar _ -> 0
  | App (t,u) | Pair(t, u) -> max (h_typ_rec h_kind x t) (h_typ_rec h_kind x u)
  | Lam (y, k, t) ->
...
let rec pre_var_map_typ_rec var_map_kind f_free = function
  ...
  | Lam (x, tau, t) ->
      Lam (x, tau, var_map_typ_rec var_map_kind f_free t)
  | Pair (t1, t2) ->
