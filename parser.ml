...
let typ_expr_parser = convert_parser Lambda_parser.typ_expr

let kind_expr_parser = convert_parser Lambda_parser.kind_expr

module String =  struct

  let raw_parse_typ s =
    let lexbuf = Ulexing.from_utf8_string s in
    typ_expr_parser (fun () -> Lambda_lexer.token "from_string" lexbuf)
  ...
  let raw_parse_kind s =
    let lexbuf = Ulexing.from_utf8_string s in
    kind_expr_parser (fun () -> Lambda_lexer.token "from_string" lexbuf)
